...
public:
  ...
  // We can reuse VisualData or similar. Let's reuse struct VisualData from Vector.hpp
  // But wait, Vector.hpp is included. 
  // We need to access VisualData definition. Ideally it should be moved to a common header.
  // For now, let's redefine similar struct or assuming we can use it if we make it public/common.
  // Actually, Vector.hpp defined it inside namespace iii.
  
  struct MatVisualData {
    size_t id = 0;
    std::string label;
    // Matrices (Frames) have basis colors usually, maybe just a main label color?
  };
  struct NoVisualData {};
...
#else
  using DataMember = typename std::conditional<Visualize, MatVisualData, NoVisualData>::type;
#endif
  ...
  static Matrix4 Rotate(T angle, const Vector3<T, Visualize>& axis) {
      ...
      // Eigen has AngleAxis
      Eigen::Matrix<T, 3, 3> rot = Eigen::AngleAxis<T>(angle, axis).toRotationMatrix();
      m.block(0,0,3,3) = rot;
      
      if constexpr (Visualize) m.record_update();
      return m;
  }
  ...
  Vector3<T, Visualize || VecVis> operator*(const Vector3<T, VecVis>& v) const {
      ...
      if constexpr (Visualize || VecVis) {
          size_t matId = 0;
          size_t vecId = 0;
          if constexpr (Visualize) matId = m_data.id;
          if constexpr (VecVis) vecId = v.m_data.id;
  ...
  Matrix4<T, Visualize || OtherVis> operator*(const Matrix4<T, OtherVis>& other) const {
      Matrix4<T, Visualize || OtherVis> result = static_cast<const Base&>(*this) * static_cast<const Base&>(other);
  ...
  void register_visual() {
    if constexpr (Visualize) {
      if (m_data.id == 0) {
        ...
        // Label defaults?
        Recorder::get().record(EventCreate{m_data.id, "Frame", {0.8,0.8,0.8}});
  ...
  void record_update() {
    if constexpr (Visualize) {
      ...
      for(int i=0; i<16; ++i) e.m[i] = static_cast<double>(data[i]);
      Recorder::get().record(e);
    }
  }

  void set_label(std::string l) {
      if constexpr (Visualize) m_data.label = l; 
